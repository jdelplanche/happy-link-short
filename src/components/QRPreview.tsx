...
export function QRPreview({
  ...
  /**
   * Mini "Export preview": composes the exact artefact the download pipeline
   * produces (same caption band, margins, frame and colours) at a small size,
   * so what you check here is byte-for-byte the same composition as PNG/JPEG/SVG.
   */
  const openExportPreview = useCallback(async () => {
    setPreviewBusy(true);
    try {
      const size = 512;
      const margin = Math.round(downloadMargin * (size / Math.max(1, downloadSize)));
      const toSvgUrl = (svg: string) =>
        `data:image/svg+xml;base64,${btoa(unescape(encodeURIComponent(svg)))}`;

      let qrHref: string;
      if (artisticStyle && matrix.bits.length) {
        qrHref = toSvgUrl(
          artisticQrSvg({
            bits: matrix.bits,
            style: artisticStyle,
        ...
        const raw = (await instance.getRawData("png")) as Blob | null;
        if (!raw) throw new Error("preview render failed");
        qrHref = await new Promise<string>((res, rej) => {
          const r = new FileReader();
          r.onload = () => res(r.result as string);
          r.onerror = rej;
          r.readAsDataURL(raw);
      ...
      console.error("[qr] export preview failed", error);
      toast({
        title: "Export preview mislukt",
        description: "Kon de exportcompositie niet opbouwen.",
        variant: "destructive",
      });
    } finally {
  ...
  return (
    <div ref={containerRef} className="flex flex-col items-center gap-6 w-full">
      ...
      {/* Mini export control: verify the exact PNG/JPEG/SVG composition. */}
      ...
      <Dialog open={Boolean(exportPreview)} onOpenChange={(o) => !o && setExportPreview(null)}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>Export preview</DialogTitle>
            <DialogDescription>
              Exact dezelfde compositie als je download — caption, marges en kleuren inbegrepen (
              {downloadFormat.toUpperCase()} · {Math.round(downloadSize)}px · marge{" "}
              {Math.round(downloadMargin)}px).
            </DialogDescription>
          </DialogHeader>
          {exportPreview && (
            <img
              src={exportPreview}
              alt="Preview of the exported QR code"
              className="mx-auto w-full max-w-[320px] rounded-lg border border-border"
            />
          )}
        </DialogContent>
      </Dialog>
